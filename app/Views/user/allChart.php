...
<h1>All Machine Charts for <?= htmlspecialchars($date) ?> - <?= htmlspecialchars($area) ?></h1>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const selectedArea = '<?= htmlspecialchars($area) ?>';
        if (selectedDate && selectedArea) {
            fetchChartData(selectedDate, selectedArea);
        }
    });

    function fetchChartData(date, area) {
        // Show loading spinner
        const loadingSpinner = document.querySelector('.loading-spinner');
        loadingSpinner.style.display = 'block';

        fetch('<?= base_url('recap/fetchChartData') ?>', {
                method: 'POST',
                headers: {
